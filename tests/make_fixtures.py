...
def unmarked_structure(path: Path) -> None:
    ...
    # Реальный провал (web01/web02): TOC с табом-лидером (а не точками) + номер.
    _set_run(doc.add_paragraph().add_run("Глава 5. Название с табуляцией\t99"), "Calibri", 12)
    ...
    # Реальный провал (doc05): глава с римским номером.
    _set_run(doc.add_paragraph().add_run("ГЛАВА II. Экспериментальная работа"), "Calibri", 12)
    # Реальный провал (doc08): одноуровневая глава — только если полужирная.
    _bold = doc.add_paragraph().add_run("1 Теоретические основы исследуемого вопроса")
    _bold.font.name = "Calibri"
    _bold.font.size = Pt(12)
    _bold.bold = True
    # Реальный провал (doc08): длинный многоуровневый подзаголовок (>100 символов).
    _set_run(
        doc.add_paragraph().add_run(
            "2.1 Система показателей для оценки эффективности управления "
            "природоохранной деятельностью нефтяной компании отрасли"
        ),
        "Calibri",
        12,
    )
